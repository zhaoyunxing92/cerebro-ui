...
export class OverviewService {
  ...
  overview(host: string): Observable<any> {
    ...
    const nodesStats = this.http.get(`${host}/_nodes/stats/jvm,fs,os,process?human=true`);
    const indexStats = this.http.get(`${host}/_stats/docs,store?ignore_unavailable=true`);
